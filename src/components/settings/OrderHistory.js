...
export default function OrderHistory() {
    ...
    const createData = data => data.map(item => ({
        ...
        status: item.status,
        date: `${item.createdAt.split('-')[1]}/${item.createdAt.split('-')[2].split('T')[0]}/${item.createdAt.split('-')[0]}`,
        total: item.total,
        id: item.id
    }))
    ...
    const columns = [
      ...
      { field: "status",  headerName: 'Status', flex: 1, renderCell: ({ value }) => <Chip label={value} classes={{label: classes.chipLabel}} />, },
      { field: "date",  headerName: 'Date', flex: 1, type: 'date' },
      { field: "total",  headerName: 'Total', flex: 1, renderCell: ({ value }) => <Chip label={`$${value}`} classes={{label: classes.chipLabel}} /> },
