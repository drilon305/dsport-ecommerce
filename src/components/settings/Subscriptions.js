...
export default function Subscriptions({ setSelectedSetting }) {
    ...
    useEffect(() => {
            ...
            }).then(response => setSubscriptions(response.data)).catch(error => {
                console.error(error)
                dispatchFeedback(setSnackbar({status: 'error', message: 'There was a problem retrieving your subscriptions. Please try again.'}))
            })
    }, [])

    const createData = data => data.map(
        ({
        shippingInfo, shippingAddress, billingInfo, billingAddress,
        paymentMethod, name, variant, quantity, frequency, next_delivery, id}) => ({
            
        details: { shippingInfo, shippingAddress, billingInfo, billingAddress, paymentMethod },
        item: { name, variant },
    ...
    return <SettingsGrid setSelectedSetting={setSelectedSetting} rows={[]} columns={columns} rowsPerPage={3} />
}